...
export default function ProductCard({ product }) {
  const { name, price, rating, category, badge, gradient, icon } = product;
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white dark:border-slate-800/80 dark:bg-slate-900/50 hover:shadow-xl hover:border-slate-300 dark:hover:border-slate-700/80 transition-all duration-300">
      ...
      <div className="relative aspect-square w-full overflow-hidden bg-slate-100 dark:bg-slate-950 flex items-center justify-center p-8">
        ...
        <div className={`relative flex h-full w-full items-center justify-center rounded-2xl bg-gradient-to-tr ${gradient} opacity-85 group-hover:opacity-100 transition-all duration-500 scale-100 group-hover:scale-105 group-hover:rotate-1 shadow-inner border border-white/5`}>
        ...
        <div className="mt-2 flex items-center gap-1">
          <div className="flex text-amber-400">
            {[...Array(5)].map((_, i) => (
              <svg
                key={i}
                className={`h-3.5 w-3.5 ${i < Math.floor(rating) ? "fill-current" : "text-slate-300 dark:text-slate-700"}`}
                viewBox="0 0 20 20"
                fill="currentColor"
              >
